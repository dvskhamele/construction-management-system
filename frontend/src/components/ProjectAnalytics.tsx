...
const ProjectAnalytics: React.FC = () => {
  ...
  const [crewRecommendations, setCrewRecommendations] = useState<StaffRecommendation[]>([])
  ...
  useEffect(() => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const actual = Math.floor(Math.random() * 30) + 20 // 20-50 tasks
      ...
      const currentCrew = 12 // Fixed current crew count
      const recommendedCrew = Math.max(8, Math.min(20, Math.floor(predicted / 4))) // Recommendation based on workload
    ...
    setCrewRecommendations(recommendations)
  }, [timeRange])

  return (
    <div className="bg-white rounded-2xl shadow-md p-6 card">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-slate-800">Project Workload Analytics</h2>
      ...
      <div className="mt-6 p-4 bg-blue-50 rounded-xl border border-blue-200">
        ...
        <ul className="list-disc pl-5 space-y-1 text-slate-700">
          <li>Based on project forecasts, you may need 3 additional crew members next week</li>
          <li>Tuesday typically has 15% higher task completion than other weekdays</li>
          <li>Consider pre-allocating resources for foundation work during rainy season</li>
        </ul>
      </div>
    </div>
  )
