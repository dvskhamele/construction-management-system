...
const SiteEmergencyResponse: React.FC = () => {
  const [devices, setDevices] = useState<SmartSiteDevice[]>([
    {
      id: 1,
      siteId: '101',
  ...
  // Get unique sites
  const sites = Array.from(new Set(devices.map(device => device.siteName))).sort()

  // Filter devices based on selected site and search term
  const filteredDevices = devices.filter(device => {
    const matchesSite = selectedSite === 'all' || device.siteName === selectedSite
    const matchesSearch = device.deviceName.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          device.siteName.includes(searchTerm)
    return matchesSite && matchesSearch
  })

  // Group devices by site
  const devicesBySite: Record<string, SmartSiteDevice[]> = {}
  filteredDevices.forEach(device => {
    if (!devicesBySite[device.siteName]) {
      devicesBySite[device.siteName] = []
    }
    devicesBySite[device.siteName].push(device)
  })
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md p-6 card">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <h2 className="text-xl font-semibold text-slate-800">Smart Site Controls</h2>
      ...
      {Object.keys(devicesBySite).length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.entries(devicesBySite).map(([siteName, siteDevices]) => (
            <div key={siteName} className="border border-slate-200 rounded-xl p-5 hover:shadow-md transition-shadow">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium text-slate-800">{siteName}</h3>
                <span className="text-xs px-2 py-1 bg-emerald-100 text-emerald-800 rounded-full">
                  {siteDevices.filter(d => d.status).length}/{siteDevices.length} Active
                </span>
              </div>
