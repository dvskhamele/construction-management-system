      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-2xl shadow-md hover:shadow-lg transition duration-300 p-6 border-l-4 border-gray-500 card cursor-pointer transform hover:-translate-y-1" onClick={() => setFilter('all')}>
            ...
            <div className="mt-4">
              <span className="text-xs text-slate-500">Value: ₹{Math.round(stats.totalValue / 100000)}L</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-md hover:shadow-lg transition duration-300 p-6 border-l-4 border-blue-500 card cursor-pointer transform hover:-translate-y-1" onClick={() => setFilter('planning')}>
            ...
            <div className="mt-4">
              <span className="text-xs text-slate-500">Avg value: ₹{Math.round(stats.avgProjectValue / 1000)}k</span>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-md hover:shadow-lg transition duration-300 p-6 border-l-4 border-indigo-500 card cursor-pointer transform hover:-translate-y-1">
            ...
            <div className="mt-4">
              ...
              <div className="flex justify-between text-xs text-slate-500 mt-1">
                <span>₹{Math.round(stats.budgetSpent / 100000)}L spent</span>
                <span>₹{Math.round(stats.budgetRemaining / 100000)}L remaining</span>
              </div>
            </div>
          </div>
                ...
                {sortedProjects.map((project) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                      ₹{Math.round(project.value / 1000)}k
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                      <div>Spent: ₹{Math.round(project.budgetSpent / 1000)}k</div>
                      <div>Remaining: ₹{Math.round(project.budgetRemaining / 1000)}k</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                      {project.issues}
                    </td>
            ...
            <div className="h-64 flex items-end space-x-2 mt-8">
              {Object.entries({
                ...
                <div key={index} className="flex flex-col items-center flex-1 group">
                  ...
                  <div className="text-xs text-slate-600 mt-2">₹{Math.round(count * 120000 / 1000)}k avg</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-md p-6 card">
            ...
            <div className="h-64 flex items-end space-x-2 mt-8">
              {sortedProjects.slice(0, 5).map((project, index) => (
                <div key={index} className="flex flex-col items-center flex-1 group">
                  <div className="text-center mb-2">
                    <div className="text-lg font-bold text-slate-800">₹{(project.budgetSpent / 100000).toFixed(1)}L</div>
