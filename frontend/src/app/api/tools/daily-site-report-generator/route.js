...
export async function POST(request) {
  try {
    ...
    const toolResultsFilePath = path.join(resultsDir, 'daily-site-report-generator.json');
    ...
    const analyticsEvent = {
      event: 'ToolSubmitted',
      tool_slug: 'daily-site-report-generator',
      timestamp,
      input_data: { input1 },
    ...
    const errorEvent = {
      event: 'ErrorEvent',
      error_code: 'DAILY-SITE-REPORT-GENERATOR_CALC_FAILED',
      message: error.message,
      severity: 'high',
      timestamp: new Date().toISOString(),
      context: { tool_slug: 'daily-site-report-generator' },
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.ip,
    };
...
export async function PUT(request) {
  try {
    ...
    const newLead = {
      ...
      lead_source: 'daily-site-report-generator',
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.ip,
    };
