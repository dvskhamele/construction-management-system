...
export async function POST(request) {
  try {
    ...
    const toolResultsFilePath = path.join(resultsDir, 'safety-compliance-checklist-generator.json');
    ...
    const analyticsEvent = {
      event: 'ToolSubmitted',
      tool_slug: 'safety-compliance-checklist-generator',
      timestamp,
      input_data: { input1 },
    ...
    const errorEvent = {
      event: 'ErrorEvent',
      error_code: 'SAFETY-COMPLIANCE-CHECKLIST-GENERATOR_CALC_FAILED',
      message: error.message,
      severity: 'high',
      timestamp: new Date().toISOString(),
      context: { tool_slug: 'safety-compliance-checklist-generator' },
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.ip,
    };
...
export async function PUT(request) {
  try {
    ...
    const newLead = {
      ...
      lead_source: 'safety-compliance-checklist-generator',
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.ip,
    };
