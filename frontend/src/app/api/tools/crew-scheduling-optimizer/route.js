...
export async function POST(request) {
  try {
    ...
    const toolResultsFilePath = path.join(resultsDir, 'crew-scheduling-optimizer.json');
    ...
    const analyticsEvent = {
      event: 'ToolSubmitted',
      tool_slug: 'crew-scheduling-optimizer',
      timestamp,
      input_data: { input1 },
    ...
    const errorEvent = {
      event: 'ErrorEvent',
      error_code: 'CREW-SCHEDULING-OPTIMIZER_CALC_FAILED',
      message: error.message,
      severity: 'high',
      timestamp: new Date().toISOString(),
      context: { tool_slug: 'crew-scheduling-optimizer' },
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.ip,
    };
...
export async function PUT(request) {
  try {
    ...
    const newLead = {
      ...
      lead_source: 'crew-scheduling-optimizer',
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.ip,
    };
