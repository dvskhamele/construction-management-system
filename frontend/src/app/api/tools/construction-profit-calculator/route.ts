...
export async function POST(request: Request) {
  try {
    ...
    // Input validation
    if (
      typeof totalContractValue !== 'number' || totalContractValue <= 0 ||
      typeof materialCosts !== 'number' || materialCosts < 0 ||
      typeof laborCosts !== 'number' || laborCosts < 0 ||
      typeof subcontractorCosts !== 'number' || subcontractorCosts < 0 ||
      typeof equipmentRentalCosts !== 'number' || equipmentRentalCosts < 0 ||
      typeof overheadPercentage !== 'number' || overheadPercentage < 0 ||
      typeof contingencyPercentage !== 'number' || contingencyPercentage < 0
    ) {
    ...
    const toolResultsFilePath = path.join(resultsDir, 'construction-profit-calculator.json');
    ...
    const analyticsEvent = {
      ...
      tool_slug: 'construction-profit-calculator',
      timestamp,
      input_data: {
        totalContractValue,
        materialCosts,
    ...
    const errorEvent = {
      event: 'ErrorEvent',
      error_code: 'PROFIT_CALC_FAILED',
      message: (error as Error).message,
      severity: 'high',
      timestamp: new Date().toISOString(),
      context: { tool_slug: 'construction-profit-calculator' },
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.headers.get('x-real-ip') || '',
    };
