...
export async function POST(request) {
  try {
    ...
    const toolResultsFilePath = path.join(resultsDir, 'risk-assessment-matrix-tool.json');
    ...
    const analyticsEvent = {
      event: 'ToolSubmitted',
      tool_slug: 'risk-assessment-matrix-tool',
      timestamp,
      input_data: { input1 },
    ...
    const errorEvent = {
      event: 'ErrorEvent',
      error_code: 'RISK-ASSESSMENT-MATRIX-TOOL_CALC_FAILED',
      message: error.message,
      severity: 'high',
      timestamp: new Date().toISOString(),
      context: { tool_slug: 'risk-assessment-matrix-tool' },
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.ip,
    };
...
export async function PUT(request) {
  try {
    ...
    const newLead = {
      ...
      lead_source: 'risk-assessment-matrix-tool',
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.ip,
    };
