...
export async function POST(request) {
  try {
    ...
    const toolResultsFilePath = path.join(resultsDir, 'equipment-utilization-tracker.json');
    ...
    const analyticsEvent = {
      event: 'ToolSubmitted',
      tool_slug: 'equipment-utilization-tracker',
      timestamp,
      input_data: { input1 },
    ...
    const errorEvent = {
      event: 'ErrorEvent',
      error_code: 'EQUIPMENT-UTILIZATION-TRACKER_CALC_FAILED',
      message: error.message,
      severity: 'high',
      timestamp: new Date().toISOString(),
      context: { tool_slug: 'equipment-utilization-tracker' },
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.ip,
    };
...
export async function PUT(request) {
  try {
    ...
    const newLead = {
      ...
      lead_source: 'equipment-utilization-tracker',
      user_agent: request.headers.get('user-agent'),
      ip_address: request.headers.get('x-forwarded-for') || request.ip,
    };
